			...
			for (int core_kernel_index = 0; core_kernel_index < core_kernel;
				...
				for (int inner_order_index = 0; inner_order_index < order;
					...
					if (inner_order_index != order_index) {
						...
						while (temp_temp != 1) {
							temp_temp /= 2;
							temp += __shfl_down_sync(0xffffffff, temp,
									temp_temp);
						}

						temp = __shfl_sync(0xffffffff, temp,
								(local_id % local) * core);
...
#pragma omp parallel
	{
		...
		for (int j = 0; j < part_per_gpu; j++) {
			...
			Update_Parameter_A_SGD_Shared<<<grid_size,
			block_size,
...
__global__ void Parameter_B_SGD_Gradient_Shared(const int order,
		...
		type_of_data **parameter_a, type_of_data **parameter_b, const int nnz,
		const type_of_data *value, const int *index,
		type_of_data *b_sum, int update_order_index) {
	...
	for (int nnz_index = worker_id; nnz_index < nnz; nnz_index += workers) {
		...
		for (int order_index = update_order_index;
				order_index < update_order_index + 1; order_index++) {
			type_of_data ho;
			for (int core_kernel_index = 0; core_kernel_index < core_kernel;
					core_kernel_index++) {
				ho = parameter_a[order_index][index[nnz_index * order
						+ order_index] * core_dimen + lane_id];
		...
		for (int core_kernel_index = 0; core_kernel_index < core_kernel;
			...
			x_r += ho_shared[core_kernel_index * block_size + threadIdx.x]
					* b_shared[update_order_index * core_kernel * core_dimen
		...
		for (int core_kernel_index = 0; core_kernel_index < core_kernel;
				core_kernel_index++) {
			atomicAdd(
					&b_sum[(nnz_index % sum_size) * order * core_kernel
							* core_dimen
							+ update_order_index * core_kernel * core_dimen
							+ core_kernel_index * core_dimen + lane_id],
					x_r
...
__global__ void Parameter_B_SGD_Gradient(const int order, const int core_kernel,
	...
	for (int nnz_index = worker_id; nnz_index < nnz; nnz_index += workers) {
		...
		for (int core_kernel_index = 0; core_kernel_index < core_kernel;
				core_kernel_index++) {
			x_r += ho_shared[core_kernel_index * block_size + threadIdx.x]
					* parameter_b[update_order_index][core_kernel_index
							* core_dimen + lane_id];
		}
		int temp = core;
		while (temp != 1) {
...
__global__ void Parameter_B_Gradient_Sum_Part(const int order,
		const int core_kernel, const int core_dimen,
...
__global__ void Parameter_B_Gradient_Sum_Whole(const int order,
		const int core_kernel, const int core_dimen, const int nnz,
		type_of_data *b_sum, type_of_data *b_grad, const int gpu_number) {
	...
	for (int b_index = worker_id; b_index < order * core_kernel; b_index +=
			workers) {
		for (int gpu_number_index = 0; gpu_number_index < gpu_number;
				gpu_number_index++) {
			b_grad[b_index * core_dimen + lane_id] += b_sum[gpu_number_index
					* order * core_kernel + b_index * core_dimen + lane_id];
		}
...
__global__ void Update_Parameter_B(const int order, const int core_kernel,
		const int core_dimen, type_of_data **parameter_b, type_of_data *b_grad,
...
#pragma omp parallel
	{
		...
		if (i == 0) {
			cudaMalloc((void**) &(mutil_b_grad[i]),
					gpu_number * order * core_kernel * core_dimen
							* sizeof(type_of_data));
		...
		for (int j = 0; j < part_per_gpu; j++) {
			...
			for (int order_index = 0; order_index < order; order_index++) {
				...
				order * core_kernel * block_size * sizeof(type_of_data)>>>(
						order, core_kernel, core_dimen, parameter_a_device[i],
						parameter_b_device[i],
		...
		order * core_kernel / (block_size / core_dimen) + 1,
		block_size>>>(order, core_kernel, core_dimen, mutil_b_sum[i],
				mutil_b_grad[i]);
		cudaDeviceSynchronize();
...
#pragma omp barrier
		if (i == 0) {
			...
			order * core_kernel / (block_size / core_dimen) + 1,
			block_size>>>(order, core_kernel, core_dimen, nnz, mutil_b_sum[0],
					mutil_b_grad[0], gpu_number);
			cudaDeviceSynchronize();

			Update_Parameter_B<<<
			order * core_kernel / (block_size / core_dimen) + 1,
			block_size>>>(order, core_kernel, core_dimen, parameter_b_device[0],
					mutil_b_grad[0], learn_rate_b, lambda_b);
			cudaDeviceSynchronize();
		...
		cudaFree(mutil_b_sum[i]);
		cudaFree(mutil_b_grad[i]);
	}
	*memcpy_time_b = 0.0;
	for (int i = 0; i < gpu_number; i++) {
