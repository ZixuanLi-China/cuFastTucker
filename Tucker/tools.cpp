...
void Cuda_Parameter_Initialization(int order, int core_length, int core_dimen,
	...
	*g_host_to_device = (type_of_data **) malloc(sizeof(type_of_data*) * order);
	*index_train_host_to_device = (int **) malloc(sizeof(int*) * data_part);
	*value_train_host_to_device = (type_of_data **) malloc(sizeof(type_of_data*) * data_part);
	...
	cudaMalloc((void**) &(*parameter_g_device),
			sizeof(type_of_data) * core_length);
	cudaMalloc((void**) &(*g_device), sizeof(type_of_data*) * order);
	...
	type_of_data* temp_value;
	cudaMalloc((void**) &temp_value, sizeof(type_of_data) * (nnz_train - (data_part - 1) * data_per_part));
	(*value_train_host_to_device)[data_part - 1] = temp_value;
	cudaMemcpy(temp_value, value_train_host[data_part - 1],
			sizeof(type_of_data) * (nnz_train - (data_part - 1) * data_per_part), cudaMemcpyHostToDevice);
	...
	cudaMemcpy(*parameter_g_device, parameter_g_host,
			sizeof(type_of_data) * core_length, cudaMemcpyHostToDevice);
	cudaMemcpy(*g_device, *g_host_to_device, sizeof(type_of_data*) * order,
			cudaMemcpyHostToDevice);
