		...
		for (int j = 0; j < part_per_gpu; j++) {
			...
			for (int update_index = 0; update_index < order; update_index++) {
				...
				order * block_size * sizeof(type_of_data)>>>(order, core_length,
						...
						learn_rate_a, lambda_a, update_index);
...
__global__ void Parameter_G_Gradient_Sum_Whole(const int core_length,
		const int core_dimen, const int nnz, const int gpu_number,
		type_of_data *g_grad) {
	...
	for (int g_index = worker_id; g_index < core_length / core_dimen; g_index +=
			workers) {
		for (int sum_size_index = 1; sum_size_index < gpu_number;
				sum_size_index++) {
			g_grad[g_index * core_dimen + lane_id] += g_grad[sum_size_index
					* core_length + g_index * core_dimen + lane_id];
		}
...
#pragma omp barrier
		if (i == 0) {
			...
			core_length / (block_size / core_dimen) + 1, block_size>>>(
					core_length, core_dimen, nnz, gpu_number, mutil_g_grad[0]);
			cudaDeviceSynchronize();
		...
		cudaFree(mutil_g_sum);
		cudaFree(mutil_g_grad);
	}
	*memcpy_time_g = 0.0;
	for (int i = 0; i < gpu_number; i++) {
