	...
	*parameter_b = (type_of_data **) malloc(sizeof(type_of_data*) * order);
	...
	for (int i = 0; i < order; i++) {
		...
		for (int j = 0; j < core_kernel * core_dimen; j++) {
			(*parameter_b)[i][j] = pow(1.0 / core_kernel, 1.0 / order) * 1.0
					* frand(0.5, 1.5);
		}
	}
	...
	*multi_value_train = (type_of_data **) malloc(
	...
	for (int i = 0; i < nnz_train; i++) {
		...
		for (int j = 0; j < order; j++) {
			...
			part_temp += (index_train[i / data_per_part][(i % data_per_part)
					* order + j] % gpu_number) * weight_temp;
		}
	...
	for (int i = 0; i < nnz_train; i++) {
		...
		for (int j = 0; j < order; j++) {
			...
			int quotient = index_train[i / data_per_part][(i % data_per_part)
					* order + j] / gpu_number;
			int remainder = index_train[i / data_per_part][(i % data_per_part)
					* order + j] % gpu_number;
...
void Mutil_Cuda_Parameter_Initialization(int order, int gpu_number,
	...
	*parameter_b_device = (type_of_data ***) malloc(
			sizeof(type_of_data**) * gpu_number);
...
#pragma omp parallel for
	for (int i = 0; i < gpu_number; i++) {
		...
		cudaMalloc((void**) &((*parameter_b_device)[i]),
				sizeof(type_of_data*) * order);
		(*parameter_b_host_to_device)[i] = (type_of_data **) malloc(
				sizeof(type_of_data*) * order);
		...
		for (int j = 0; j < order; j++) {
			...
			cudaMalloc((void**) &temp_b,
					sizeof(type_of_data) * dimen[j] * core_dimen);
			(*parameter_b_host_to_device)[i][j] = temp_b;
			cudaMemcpy(temp_b, parameter_b_host[j],
		...
		cudaMemcpy((*parameter_b_device)[i], (*parameter_b_host_to_device)[i],
				sizeof(type_of_data*) * order, cudaMemcpyHostToDevice);
